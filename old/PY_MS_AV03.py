...
"""
    ...
    Notes:	
		v0.2.1 	
		Changed SNMP Access to receive all values in 1 request.
		This however did not really improve the performance of the requests
		This has made a difference, about 2x faster. 198 CM in 3:32
		(before 07:55 / 198 CM)

		I have also changed the design a bit by moving some methods 
		to the cm class

		The US/DS Values for bonded US/DS have now been splitted and
		added as a list to the cm attributes.
		
		1. Do I really need a filehandle in class telnet
		   as this could cause an issue with Multthreading
		   later on?

		v0.3
		Added CGI Functionality
		Right now this works only for a complete mac domain (C5/0/4),
		not for single US !
